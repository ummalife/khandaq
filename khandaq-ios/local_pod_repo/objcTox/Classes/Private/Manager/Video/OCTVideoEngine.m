                      ...
                      friendNumber:(OCTToxFriendNumber)friendNumber
{
    ...
    /* Copy yPlane data; zero stride padding to avoid green artifact at right edge */
    for (size_t yHeight = 0; yHeight < height; yHeight++) {
        memcpy(yDestinationPlane, ySource, yBytesPerRow);
        if (yDestinationStride > yBytesPerRow) {
            memset(yDestinationPlane + yBytesPerRow, 0, yDestinationStride - yBytesPerRow);
        }
        ySource += yStride;
    ...
    const size_t uvRowBytes = uvBytesPerRow * 2;
    for (size_t yHeight = 0; yHeight < height / 2; yHeight++) {
        ...
        if (uvDestinationStride > uvRowBytes) {
            memset(uvDestinationPlane + uvRowBytes, 128, uvDestinationStride - uvRowBytes);
        }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    const size_t frameWidth = CVPixelBufferGetWidthOfPlane(imageBuffer, 0);
    const size_t frameHeight = CVPixelBufferGetHeightOfPlane(imageBuffer, 0);
    const size_t chromaWidth = frameWidth / 2;
    const size_t chromaHeight = frameHeight / 2;
    ...
    const size_t ySize = frameWidth * frameHeight;
    const size_t numberOfElementsForChroma = chromaWidth * chromaHeight;
    ...
    /**
     * Pack NV12 into tight I420 planes (toxav expects width = frame pixels, not row stride).
     */
    OCTToxAVPlaneData *yPlane = CVPixelBufferGetBaseAddressOfPlane(imageBuffer, 0);
    uint8_t *yDestination = self.reusableYChromaPlane;
    for (size_t row = 0; row < frameHeight; row++) {
        memcpy(yDestination, yPlane, frameWidth);
        yPlane += yBytesPerRow;
        yDestination += frameWidth;
    }
    ...
    for (size_t row = 0; row < chromaHeight; row++) {
        for (size_t i = 0; i < chromaWidth; i++) {
            uDestination[i] = uvPlane[i * 2];
            vDestination[i] = uvPlane[i * 2 + 1];
        }
        uvPlane += uvBytesPerRow;
        uDestination += chromaWidth;
        vDestination += chromaWidth;
    }
    ...
    if (! [self.toxav sendVideoFrametoFriend:self.friendNumber
                                       width:(OCTToxAVVideoWidth)frameWidth
                                      height:(OCTToxAVVideoHeight)frameHeight
                                      yPlane:self.reusableYChromaPlane
                                      uPlane:self.reusableUChromaPlane
                                      vPlane:self.reusableVChromaPlane
                                       error:&error]) {
        OCTLogWarn(@"error:%@ width:%zu height:%zu", error, frameWidth, frameHeight);
    }
}
