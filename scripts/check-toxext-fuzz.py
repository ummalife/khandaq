#!/usr/bin/env python3
"""Fuzz the toxext messages parser — the first code a contact's packet reaches.
...
into testing a copy.
    ...
    scripts/check-toxext-fuzz.py
    scripts/check-toxext-fuzz.py --libfuzzer --seconds 45
"""
...
HARNESS = os.path.join(HERE, "toxext_fuzz_harness.c.in")
SRC = os.path.join(ROOT, "khandaq-desktop/buildscripts/toxext_messages/tox_extension_messages.c")
...
def extract_function(src, name, where):
    m = re.search(r"^bool\s+%s\s*\(" % re.escape(name), src, re.M)
    if not m:
        die("%s: cannot find `bool %s(` — the file changed shape and this check would have become "
...
def main():
    ...
    if not os.path.isfile(SRC):
        die("не найден tox_extension_messages.c — проверять нечего, а пустой прогон выглядел бы "
            "как успех")

    where = os.path.relpath(SRC, ROOT)
    fn = extract_function(read(SRC), "parse_messages_packet", where)
    ...
    if use_lf:
        ...
        print("    детерминированный корпус, пакетов: %d" % args.iterations)
    ...
    print("    %s" % tail[0])
    print("\nВСЁ ЧИСТО: сгенерированные пакеты не вывели парсер за пределы буфера")
    return 0
