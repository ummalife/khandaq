...
constexpr float DEFAULT_SCREEN_CAPTURE_FPS = 24.0f;
constexpr float DEFAULT_CAMERA_CAPTURE_FPS = 5.0f;
constexpr int MAX_SCREEN_CAPTURE_WIDTH = 1920;
constexpr int MAX_SCREEN_CAPTURE_HEIGHT = 1080;

QSize cappedScreenCaptureSize(int width, int height)
{
    if (width <= 0 || height <= 0) {
        return {};
    }

    if (width <= MAX_SCREEN_CAPTURE_WIDTH && height <= MAX_SCREEN_CAPTURE_HEIGHT) {
        return {width, height};
    }

    const double scale = std::min(static_cast<double>(MAX_SCREEN_CAPTURE_WIDTH) / width,
                                  static_cast<double>(MAX_SCREEN_CAPTURE_HEIGHT) / height);
    return {(static_cast<int>(width * scale) / 2) * 2, (static_cast<int>(height * scale) / 2) * 2};
}
} // namespace
...
CameraDevice* CameraDevice::open(QString devName, VideoMode mode)
{
    ...
    const bool screenCapture = devName.startsWith("x11grab#") || devName.startsWith("gdigrab#");
    float FPS = screenCapture ? DEFAULT_SCREEN_CAPTURE_FPS : DEFAULT_CAMERA_CAPTURE_FPS;
    if (mode.FPS > 0.0f) {
        FPS = mode.FPS;
    ...
    if (!iformat)
        ...
        if (mode.width && mode.height) {
            const QSize cappedSize = cappedScreenCaptureSize(mode.width, mode.height);
            screen.setWidth(cappedSize.isEmpty() ? mode.width : cappedSize.width());
            screen.setHeight(cappedSize.isEmpty() ? mode.height : cappedSize.height());
        } else {
        ...
        int captureWidth = mode.width;
        int captureHeight = mode.height;
        const QSize cappedSize = cappedScreenCaptureSize(captureWidth, captureHeight);
        if (!cappedSize.isEmpty()) {
            captureWidth = cappedSize.width();
            captureHeight = cappedSize.height();
        }

        const std::string captureVideoSize =
            QStringLiteral("%1x%2").arg(captureWidth).arg(captureHeight).toStdString();
        ...
        av_dict_set(&options, "video_size", captureVideoSize.c_str(), 0);
...
QVector<VideoMode> CameraDevice::getScreenModes()
{
    ...
    std::for_each(screens.begin(), screens.end(), [&result](QScreen* s) {
        ...
        int width = static_cast<int>(rect.width() * pixRatio);
        int height = static_cast<int>(rect.height() * pixRatio);
        const QSize cappedSize = cappedScreenCaptureSize(width, height);
        if (!cappedSize.isEmpty()) {
            width = cappedSize.width();
            height = cappedSize.height();
        }

        VideoMode mode(width, height, static_cast<int>(p.x() * pixRatio),
                       static_cast<int>(p.y() * pixRatio), DEFAULT_SCREEN_CAPTURE_FPS);
