...
| генерация и хранение HSK | — не нужно | ❌ | ❌ | — |
| объявление ключа (эмиссия) | — | ❌ | ❌ | — |
| эмиссия подписанной истории | — | ❌ | ❌ | — |

Тестов: **93 JVM + 8 инструментальных (Android)**, **25 QtTest (десктоп)**, harness'ы на iOS. Ни одна из новых веток не вызывается: версию `0x02` не шлёт ни один отгруженный клиент, и все три отгруженных парсера её отбрасывают, поэтому радиус поражения всего сделанного — нулевой.
...
2. **Генерация и хранение HSK.** На Android блокер снят: `crypto_sign_keypair` доступен и проверен на устройстве — остаётся сама генерация и хранилище. На iOS libsodium доступен, но хранение — это схема Realm, то есть миграция; в этом проекте миграции уже ломали профили, так что делать её надо отдельным заходом с device QA.
