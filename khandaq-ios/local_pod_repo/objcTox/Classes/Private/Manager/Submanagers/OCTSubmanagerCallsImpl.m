...
const OCTToxAVVideoBitRate kDefaultVideoBitRate = 4000;
...
@synthesize callKitAudioSessionIsActive = _callKitAudioSessionIsActive;
...
{
    if (start) {
        OCTFriend *friend = [call.chat.friends firstObject] ?: [call.chat.friends lastObject];

        self.audioEngine.friendNumber = friend.friendNumber;
        self.videoEngine.friendNumber = friend.friendNumber;

        NSError *error;
        BOOL reconfigureSession = ! self.callKitAudioSessionIsActive;
        if (! [self.audioEngine startAudioFlowForVideoCall:call.videoIsEnabled
                                        reconfigureSession:reconfigureSession
                                                     error:&error]) {
            OCTLogError(@"Error starting audio flow: %@", error);
        }
        self.callKitAudioSessionIsActive = NO;
