...
import static com.zoffcc.applications.trifa.HelperMsgNotification.extract_open_chat_key_from_intent;
import static com.zoffcc.applications.trifa.HelperMsgNotification.store_pending_open_chat_key;
...
public class StartMainActivityWrapper extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        try
        {
            final String open_chat_key = extract_open_chat_key_from_intent(getIntent());
            if (open_chat_key != null)
            {
                store_pending_open_chat_key(open_chat_key);
            }
    ...
    protected void onNewIntent(Intent intent)
    {
        ...
        try
        {
            final String open_chat_key = extract_open_chat_key_from_intent(intent);
            if (open_chat_key != null)
            {
                store_pending_open_chat_key(open_chat_key);
            }
            if (MainActivity.main_activity_s != null)
            {
                HelperMsgNotification.consume_pending_open_chat(MainActivity.main_activity_s);
            }
        }
        catch (Exception e)
        {
            Log.i(TAG, "onNewIntent:EE:" + e.getMessage());
        }
    }
}
