...
// chain to the ISRG roots is rejected. The roots survive routine LE leaf/intermediate rotation.
// Refresh the embedded roots + expiry in a client release BEFORE the expiry date.
//
// KHANDAQ (audit 2026-08-21, K-08): the note here used to read "the roots (valid to 2035)", which was
// wrong in the direction that matters. Re-derived from the DER below: X1 notAfter 2035-06-04,
// X2 2040-09-17, Root YE 2032-09-02. Root YE — the anchor the chain is MIGRATING TO — is the
// shortest-lived of the three, the opposite of what "valid to 2035" suggests to whoever next has to
// judge how urgent a refresh is. scripts/check-push-ca-anchors.py now fails CI when this set drifts
// from the Android pin-set, when the two copies of the expiry date disagree, or when the expiry comes
// within 120 days.
