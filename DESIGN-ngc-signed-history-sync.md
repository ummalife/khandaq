...
| генерация и хранение HSK | — не нужно | ✅ код + 9 тестов | ❌ | `NgcHskStore`: g_opts, без миграции схемы; сама запись в g_opts на устройстве не проверена |
| объявление ключа (эмиссия) | — | ❌ | ❌ | — |
| эмиссия подписанной истории | — | ❌ | ❌ | — |

Тестов: **102 JVM + 8 инструментальных (Android)**, **25 QtTest (десктоп)**, harness'ы на iOS. Ни одна из новых веток не вызывается: версию `0x02` не шлёт ни один отгруженный клиент, и все три отгруженных парсера её отбрасывают, поэтому радиус поражения всего сделанного — нулевой.
...
2. **Генерация и хранение HSK.** **Android сделан** (`NgcHskStore`, `8540299b`): ключ лежит в существующих строках `g_opts` — новая таблица потребовала бы миграции схемы orma/SQLCipher, а миграции здесь уже уничтожали профили (#244). Привязан к публичному ключу Tox (не к Tox ID: nospam меняется без смены личности), строка-владелец пишется последней и служит отметкой фиксации, рассогласование пары ловится сверкой публичного ключа с хвостом секретного, результат перечитывается — `set_g_opts` глушит свои ошибки. **Не проверено на устройстве:** сама запись/чтение g_opts (нужен открытый профиль, которого у инструментального теста нет). На iOS libsodium доступен, но хранение — это схема Realm, то есть миграция; в этом проекте миграции уже ломали профили, так что делать её надо отдельным заходом с device QA.
