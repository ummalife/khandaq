...
namespace {
const QString versionUrl{QStringLiteral("https://api.github.com/repos/ummalife/khandaq/releases/latest")};
...
UpdateCheck::UpdateCheck(const Settings& settings_)
    : settings(settings_)
{
    qInfo() << "Khandaq is running version:" << GIT_DESCRIBE;
...
void UpdateCheck::checkForUpdate()
{
    ...
    if (isCurrentVersionStable() == false) {
        qWarning() << "Currently running an untested/unstable version of Khandaq";
