...
| проверка подписи | ✅ | ✅ код есть | ✅ | libsodium; Android — через JNI, символов нет в `.so` до прогона CI (запущен) |
...
Тестов: **89 JVM (Android)**, **25 QtTest (десктоп)**, harness'ы на iOS. Ни одна из новых веток не вызывается: версию `0x02` не шлёт ни один отгруженный клиент, и все три отгруженных парсера её отбрасывают, поэтому радиус поражения всего сделанного — нулевой.

**Что нужно каждому оставшемуся шагу — и это не «время», а конкретные внешние вещи:**

1. **Прогон `android-native-so.yml`** (4 ABI). Без него нужных символов нет в `libjni-c-toxcore.so`, и любой вызов даст `UnsatisfiedLinkError`. **Запущен 15 авг** (run `31890361557`). JNI экспортирует все три символа сразу — `khandaq_ed25519_verify`, `khandaq_ed25519_keypair`, `khandaq_ed25519_sign` (`241b0a53`): первый прогон в тот день пришлось отменить, потому что собирал бы только `verify` и оставил бы п.2 всё так же заблокированным — на 4-ABI NDK-сборке это стоит часы, а не перекомпиляцию.
2. **Генерация и хранение HSK.** На Android упиралось в п.1 (нужен `crypto_sign_keypair`) — символ теперь экспортируется, остаётся дождаться артефакта. На iOS libsodium доступен, но хранение — это схема Realm, то есть миграция; в этом проекте миграции уже ломали профили, так что делать её надо отдельным заходом с device QA.
