...
void LoginScreen::onLoginUsernameSelected(const QString& name)
{
    ...
    if (Profile::isEncrypted(name, settings.getPaths())) {
        ...
        ui->autoLoginCB->setToolTip(
            tr("Remember password in the system keychain and log in automatically."));
    } else {
        ui->loginPasswordLabel->hide();
        ui->loginPassword->hide();
        ui->autoLoginCB->show();
        ui->autoLoginCB->setToolTip(
            tr("If the profile does not have a password, Khandaq can skip the login screen."));
    }
}
...
void LoginScreen::onAutoLoginCheckboxChanged(int state)
{
    ...
    const bool enabled = cstate == Qt::CheckState::Checked;
    emit autoLoginChanged(enabled);
    if (!enabled && CredentialStore::isSupported()) {
