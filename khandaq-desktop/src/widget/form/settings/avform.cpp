    ...
    , camera(camera_)
{
    ...
    cbEnableMicTest->setToolTip(
        tr("Listen to your microphone while adjusting gain. Use headphones to avoid echo."));
...
{
    cbEnableMicTest->setChecked(false);
    setMicTestEnabled(false);
    audioSink.reset();
    audioSrc.reset();
...
void AVForm::showEvent(QShowEvent* event)
{
    ...
    if (cbEnableMicTest->isChecked()) {
        setMicTestEnabled(true);
    }
...
void AVForm::fillScreenModesComboBox()
{
    ...
    for (int i = 0; i < videoModes.size(); ++i) {
        ...
        if (mode.width && mode.height) {
            name = tr("Screen %1 (%2 fps)").arg(i + 1).arg(static_cast<int>(mode.FPS));
        } else {
...
void AVForm::on_inDevCombobox_currentIndexChanged(int deviceIndex)
{
    ...
    cbEnableMicTest->setEnabled(inputEnabled);
    if (!inputEnabled) {
...
void AVForm::on_cbEnableMicTest_stateChanged()
{
    setMicTestEnabled(cbEnableMicTest->isChecked());
}

void AVForm::setMicTestEnabled(bool enabled)
{
    if (micTestConnection) {
        disconnect(micTestConnection);
        micTestConnection = {};
    }

    if (!enabled || !audioSrc || !audioSink) {
        return;
    }

    micTestConnection = connect(
        audioSrc.get(), &IAudioSource::frameAvailable, this,
        [this](const int16_t* pcm, size_t samples, uint8_t channels, uint32_t rate) {
            if (cbEnableMicTest->isChecked() && audioSink) {
                audioSink->playAudioBuffer(pcm, static_cast<int>(samples), channels,
