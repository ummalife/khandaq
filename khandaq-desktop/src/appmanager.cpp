...
int AppManager::run()
{
    ...
    qapp->setApplicationName("Khandaq");
    qapp->setOrganizationName("Khandaq");
    qapp->setOrganizationDomain("khandaq.org");
#if QT_VERSION >= QT_VERSION_CHECK(5, 7, 0)
    qapp->setDesktopFileName("org.khandaq.messenger");
#endif
    ...
    parser.setApplicationDescription("Khandaq Messenger, version: " + QString(GIT_VERSION));
    ...
    if (QFileInfo(logfile).size() > 1000000) {
        ...
        if (dir.remove(logFileDir + "khandaq.log.1"))
            qDebug() << "Removed old log successfully";
        else
            qWarning() << "Unable to remove old log file";

        if (!dir.rename(logFileDir + "khandaq.log", logFileDir + "khandaq.log.1"))
    ...
    if (doIpc && !ipc->isCurrentOwner()) {
        ...
        if (ipc->waitUntilAccepted(event, 2)) {
            if (eventType == "activate") {
                qDebug()
                    << "Another Khandaq instance is already running. If you want to start a second "
    ...
    if (autoLogin && Profile::exists(profileName, settings->getPaths())) {
        ...
        const bool encrypted = Profile::isEncrypted(profileName, settings->getPaths());
        const bool canAutoLogin =
            !encrypted || (CredentialStore::isSupported()
                           && CredentialStore::load(profileName, loginPassword));

        if (canAutoLogin) {
            profile = Profile::loadProfile(profileName, loginPassword, *settings, &parser,
                                           *cameraSource, *messageBoxManager);
        }
