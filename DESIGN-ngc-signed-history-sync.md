...
## 0. Состояние на 13 авг 2026 — что готово и что каждому оставшемуся шагу нужно

| Компонент | десктоп | Android | iOS | чем проверено |
|---|---|---|---|---|
| pre-image (§4.3) | ✅ | ✅ | ✅ | 8 замороженных векторов, одинаковые байты у всех трёх + Python-эталон |
| проверка подписи | ✅ | ✅ **проверено на устройстве** | ✅ | libsodium; на Android JNI прогнан на эмуляторе arm64 — 8 инструментальных тестов |
| разбор `0x02` (§4.2–4.3) | ✅ | ✅ | ✅ | одни и те же 14 граничных случаев на каждой платформе |
| сборка `0x02` | не нужна | ✅ | ✅ | round-trip + **побайтовая сверка Android↔iOS** |
| пометка «не подтверждён» (§4.5) | — | ✅ в продукте | — | тесты + device QA (ложных меток нет) |
| version-gate (§5) | ✅ | ✅ | ✅ | подтверждён чтением всех трёх парсеров |
| генерация и хранение HSK | — не нужно | ✅ **проверено на устройстве** | ❌ | `NgcHskStore`: g_opts, без миграции схемы; 9 юнит + **4 инструментальных против живого профиля** |
| объявление ключа (эмиссия) | — | ❌ | ❌ | — |
| эмиссия подписанной истории | — | ✅ | ✅ | round-trip + побайтовая сверка Android↔iOS |
| **анти-даунгрейд (K-01)** | n/a — только файлы | ✅ | ✅ | 12 JUnit + та же таблица против ObjC-исходника |

Тестов: **102 JVM + 12 инструментальных (Android)**, **25 QtTest (десктоп)**, harness'ы на iOS. Ни одна из новых веток не вызывается: версию `0x02` не шлёт ни один отгруженный клиент, и все три отгруженных парсера её отбрасывают, поэтому радиус поражения всего сделанного — нулевой.

**Что нужно каждому оставшемуся шагу — и это не «время», а конкретные внешние вещи:**

1. ~~**Прогон `android-native-so.yml`**~~ — **СДЕЛАНО 15 авг** (run `31890361557`, зелёный). Все три символа (`khandaq_ed25519_verify`, `khandaq_ed25519_keypair`, `khandaq_ed25519_sign`) присутствуют во всех 4 ABI. Первый прогон в тот день отменён намеренно: он собрал бы только `verify` и оставил бы п.2 заблокированным, а это часы, а не перекомпиляция.

   Проверено **на устройстве**, а не по наличию символа: `NgcHistSigNativeTest`, 8 инструментальных тестов на эмуляторе arm64 — символы резолвятся из Java, sign↔verify сходятся, подпись детерминирована, преимидж хешируется в замороженный вектор `ascii-basic` (то есть Android подписывает те же байты, что десктоп и iOS), все некорректные входы падают закрыто.

   **`.so` в git не лежит** (gitignored): артефакт из этого прогона надо положить в `app/nativelibs`, раскладывая `armeabi` → `armeabi-v7a`. Переименование корректно, а не деградация — `deps.sh:388` собирает этот ABI с `--arch=arm --cpu=armv7-a`, каталог просто назван неудачно.
2. **Генерация и хранение HSK.** **Android сделан** (`NgcHskStore`, `8540299b`): ключ лежит в существующих строках `g_opts` — новая таблица потребовала бы миграции схемы orma/SQLCipher, а миграции здесь уже уничтожали профили (#244). Привязан к публичному ключу Tox (не к Tox ID: nospam меняется без смены личности), строка-владелец пишется последней и служит отметкой фиксации, рассогласование пары ловится сверкой публичного ключа с хвостом секретного, результат перечитывается — `set_g_opts` глушит свои ошибки. **Проверено на устройстве** (`NgcHskStoreDeviceTest`, 4 теста против живого профиля на эмуляторе): ключ генерируется один раз и перечитывается побайтово, перечитанный ключ по-прежнему подписывает и проверяется (то есть hex-круг через SQLCipher байты не портит), смена личности заменяет ключ, без личности ничего не выпускается. На iOS libsodium доступен, но хранение — это схема Realm, то есть миграция; в этом проекте миграции уже ломали профили, так что делать её надо отдельным заходом с device QA.
3. **Эмиссия** (объявление + подписанная история). Первый шаг, меняющий исходящий трафик. Раньше проверить было нечем: NGC-пир между двумя телефонами поднимался ненадёжно и лечился только перезапуском приложения (QA 11 авг). **Причина найдена и устранена 15 авг** (#246, `f0bbd9c5`): группа доходила до CONNECTED и теряла пиров в одну сторону, а `kickstart_group_connection` переподключался только при `conn == ERROR`, поэтому эскалация каждые 180 с уходила в холостую ветку. Добавлен второй, узкий проход (`NgcStuckGroupPolicy`), куда CONNECTING попасть не может. **Осталось подтвердить на двух телефонах** — состояние надо выдержать ≥5 мин, поэтому QA небыстрое; до этого подтверждения считать блокер снятым нельзя. Писать эмиссию, которую нельзя ни включить, ни проверить, — значит отгружать непроверенный код в мессенджер.
4. **Переходный период** (§8 шаги 5–6) — по релизу каждый, после того как выкатятся подписывающие клиенты.
...
**Разбор пакетов версии `0x02`: Android и десктоп готовы** — `NgcHistSigParser` (Java, 20 тестов, включая round-trip против сборщиков) и `NgcHistSig::parseAnnouncement`/`parseSignedText` (C++, те же границы, 25 тестов в `test_ngchistsig`). Границы одинаковые в обоих: точная длина у объявления, беззнаковое чтение длины текста, потолок 37000 (тот же, что у неподписанного пути), и требование, чтобы подпись шла ровно за телом без запаса. **iOS тоже готов** — `OCTNgcHistSigParseAnnouncement`/`ParseSignedText` с теми же границами, проверены отдельным harness'ом на тех же 14 случаях. **Разбор пакетов версии `0x02` реализован и покрыт тестами на всех трёх платформах.**

**Сборка пакетов: Android и iOS, и они дают БАЙТ-В-БАЙТ одинаковый результат.** Hex, который выдаёт iOS-сборщик на фиксированных входах, закреплён тестами в Android-наборе (`announcementBytesMatchTheIosBuilder`, `signedTextBytesMatchTheIosBuilder`). Это то, что не даёт двум сборщикам разъехаться: изменение раскладки на одной платформе и не на другой даёт пакеты, которые вторая сторона отвергает, и всплывает это только в поле, между двумя пользователями. Десктопу сборщик не нужен — он только потребитель истории.
...
## 10. Шаг 6, но по авторам, а не флаг-дэем (внешний аудит 21 авг 2026, K-01)

**Сделано.** Аудит нашёл ровно то, что §8 шаг 6 откладывал: пока неподписанная форма принимается
безусловно, атакующему достаточно **не отправлять** подписанную. Приём `0x02` рядом с `0x01` — это не
защита, а выбор атакующего.

Правило вместо флаг-дэя: **как только ключ автора выучен из живого, аутентифицированного транспортом
объявления в этой группе, неподписанная запись, заявляющая этого автора, отвергается.** Не «метится
как непроверенная» — отвергается. Это строго лучше флаг-дэя из шага 6: включается по каждому автору
отдельно, по мере того как он обновляется, а не одномоментно для всего парка.

`NgcHistoryDowngradePolicy` (Java) и `OCTNgcHistoryDowngradePolicy` (ObjC) — одна и та же чистая
функция дважды; `scripts/check-ios-downgrade-policy.py` компилирует тело ObjC как C и прогоняет ту же
таблицу истинности, что и JUnit, потому что «выглядит одинаково» — не доказательство, особенно когда
у одной стороны знаковый `long`, а у другой `uint64_t`.

**§5 пришлось развернуть, и это надо сказать прямо.** Документ рекомендовал не слать обе версии;
реализация всё равно шлёт обе — осознанно, чтобы не ломать историю у отгруженных клиентов. Новое
правило возвращает часть того, от чего дуал-эмит защищал: **порядок отправки перевёрнут — подписанная
копия идёт ПЕРВОЙ.** Иначе вердикт приходит на несколько миллисекунд позже неподписанного двойника
(померено на устройстве), и получатель отверг бы каждое честно подписанное сообщение в группе.

**Цена, которую нужно знать до релиза.** Ретранслятор не может изготовить чужую подпись. Значит, как
только ключ автора A известен в группе, история A доходит только напрямую от A: новичок, зашедший
пока A офлайн, историю A не получит. Это неизбежно следует из формулировки «отвергать неподписанную
историю, заявляющую этого автора», и §5 уже принимал потерю такой же формы, рекомендуя против
дуал-эмита. Единственная настройка — `KEY_STALE_MS`.

**Что не задето.** Живые сообщения не идут через этот путь. Автор, никогда не объявлявший ключ,
не задет вообще (`ACCEPT_LEGACY`) — сегодня это подавляющее большинство. Переустановка обычно даёт
новый публичный ключ в группе, то есть другую строку каталога, то есть тоже `ACCEPT_LEGACY`; худший
случай для того же ключа ограничен `REPLACE_GRACE_MS` (24 ч), после чего каталог и так готов принять
замену.

**Закрыт и второй канал.** На Android история приходит ещё и через toxproxy-реле
(`MainActivity.friend_sync_message_v2_cb`), где автор — 64 символа из тела сообщения ДРУГА, без магии,
без версии и без какой-либо подписанной формы. Он обходил `handle_incoming_sync_group_message`
целиком, и калитка только на `0x01` оставила бы более дешёвый канал подделки открытым.

**§4.5, вторая половина.** «Всё, что не VERIFIED, исключается из уведомлений» не было реализовано:
синхронизированная строка поднимала heads-up с именем заявленного автора. Теперь уведомление
разрешено, только если подпись доказала авторство ИЛИ приславший пир и есть заявленный автор (тогда
атрибуцию аутентифицировал транспорт). Строка по-прежнему сохраняется и отображается с оранжевой
меткой. iOS уже не уведомлял по синхронизированным строкам вовсе; десктоп уведомлял — по файлам
(`0x03`), единственной синхронизированной форме, которую он разбирает, — и теперь тоже не уведомляет.
