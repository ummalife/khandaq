...
bool isGenericDefaultUsername(const QString& name)
{
    ...
    if (trimmed.compare(QLatin1String("Khandaq"), Qt::CaseInsensitive) == 0) {
        return true;
    }

    if (trimmed.length() >= 5
        && trimmed.left(5).compare(QLatin1String("TRIfA"), Qt::CaseInsensitive) == 0) {
        return true;
    }
...
QString shortPublicKeyLabel(const ToxPk& friendPk)
{
    const QString full = friendPk.toString();
    if (full.length() < 6) {
        return full.toUpper();
    }

    return full.right(6).toUpper();
}
...
Friend::Friend(uint32_t friendId_, const ToxPk& friendPk_, const QString& userAlias_, const QString& userName_)
    ...
    , isNegotiating{false}
{
    if (userName_.isEmpty() || isGenericDefaultUsername(userName_)) {
        userName = shortPublicKeyLabel(friendPk_);
    } else {
        userName = userName_;
    }
}
...
{
    QString name = _name;
    if (name.isEmpty() || isGenericDefaultUsername(name)) {
        name = shortPublicKeyLabel(friendPk);
    }
