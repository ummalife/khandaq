...
Тестов: **79 JVM (Android)**, **25 QtTest (десктоп)**, harness'ы на iOS. Ни одна из новых веток не вызывается: версию `0x02` не шлёт ни один отгруженный клиент, и все три отгруженных парсера её отбрасывают, поэтому радиус поражения всего сделанного — нулевой.
...
3. **Эмиссия** (объявление + подписанная история). Первый шаг, меняющий исходящий трафик. Проверить нечем: NGC-пир между двумя телефонами поднимается ненадёжно и лечится только перезапуском приложения — задокументировано в ходе QA 11 авг. Писать эмиссию, которую нельзя ни включить, ни проверить, — значит отгружать непроверенный код в мессенджер.
