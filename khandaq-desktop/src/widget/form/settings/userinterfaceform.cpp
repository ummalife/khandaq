...
void UserInterfaceForm::on_btnResetChatFont_clicked()
{
    const QFont defaultFont = Style::getFont(Style::Font::Big);
    const int defaultSize = QFontInfo(defaultFont).pixelSize();

    const QSignalBlocker fontBlocker(bodyUI->txtChatFont);
    const QSignalBlocker sizeBlocker(bodyUI->txtChatFontSize);

    bodyUI->txtChatFont->setCurrentFont(defaultFont);
    bodyUI->txtChatFontSize->setValue(defaultSize);
    settings.setChatMessageFont(defaultFont);
}
