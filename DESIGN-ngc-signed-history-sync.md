...
| проверка подписи | ✅ | ✅ код есть | ✅ | libsodium; Android — через JNI, символа нет в `.so` до прогона CI |
...
1. **Прогон `android-native-so.yml`** (4 ABI). Без него `khandaq_ed25519_verify` отсутствует в `libjni-c-toxcore.so`, и любой вызов даст `UnsatisfiedLinkError`. Код в `jni-c-toxcore/jni-c-toxcore.c` готов и компилируется чисто.
2. **Генерация и хранение HSK.** На Android упирается в п.1 (нужен `crypto_sign_keypair`). На iOS libsodium доступен, но хранение — это схема Realm, то есть миграция; в этом проекте миграции уже ломали профили, так что делать её надо отдельным заходом с device QA.
