...
#META
###################
!define APP_NAME "Khandaq"
!define COMP_NAME "Khandaq"
!define WEB_SITE "https://khandaq.org"
!define VERSION "0.1.0.0"
!define DESCRIPTION "Khandaq Messenger Installer"
!define COPYRIGHT "Khandaq Project"
!define INSTALLER_NAME "Khandaq-installer.exe"
!define MAIN_APP_EXE "bin\Khandaq.exe"
!define REG_ROOT "HKLM"
!define REG_APP_PATH "Software\Microsoft\Windows\CurrentVersion\App Paths\Khandaq.exe"
...
!ifdef REG_START_MENU
  ...
  !define MUI_STARTMENUPAGE_DEFAULTFOLDER "Khandaq"
...
${CreateShortcut} "$DESKTOP\Khandaq.lnk" "$INSTDIR\${MAIN_APP_EXE}" "" "" ""
FunctionEnd
...
!define MUI_FINISHPAGE_RUN_FUNCTION Launch_Khandaq_without_Admin
!define MUI_FINISHPAGE_RUN
!define MUI_FINISHPAGE_LINK "Khandaq Messenger"
!define MUI_FINISHPAGE_LINK_LOCATION "https://khandaq.org"
!insertmacro MUI_PAGE_FINISH

Function Launch_Khandaq_without_Admin
   SetOutPath $INSTDIR
...
###################
  ...
  Section "Create install directory"
    ...
    FileClose $0
    FileOpen $0 "$TEMP\Khandaq-install-file-permissions.txt" w
      FileWriteUTF16LE  $0 "$INSTDIR"
      FileWriteUTF16LE  $0 "$\r$\n"
      DetailPrint "Writing to file: $2"
      FileWriteUTF16LE  $0 "$2"
    FileClose $0
    nsExec::Exec 'icacls "" /restore "$TEMP\Khandaq-install-file-permissions.txt"'
  SectionEnd
...
Section "Install"
  ...
  File /nonfatal "khandaq\imageformats\*.*"
  ${SetOutPath} "$INSTDIR\bin"

  ${CreateDirectory} "$INSTDIR\bin\iconengines"
  ${SetOutPath} "$INSTDIR\bin\iconengines"
  File /nonfatal "khandaq\iconengines\*.*"
  ${SetOutPath} "$INSTDIR\bin"

  ${CreateDirectory} "$INSTDIR\bin\platforms"
  ${SetOutPath} "$INSTDIR\bin\platforms"
  File /nonfatal "khandaq\platforms\*.*"
  ${SetOutPath} "$INSTDIR\bin"

  ${CreateDirectory} "$INSTDIR\bin\libsnore-qt5"
  ${SetOutPath} "$INSTDIR\bin\libsnore-qt5"
  File /nonfatal "khandaq\libsnore-qt5\*.*"
  ${SetOutPath} "$INSTDIR\bin"

  # Create shortcuts
  ${CreateDirectory} "$SMPROGRAMS\Khandaq"
  ${CreateShortCut} "$SMPROGRAMS\Khandaq\Khandaq.lnk" "$INSTDIR\${MAIN_APP_EXE}" "" "" ""
  ${CreateShortCut} "$SMPROGRAMS\Khandaq\Uninstall Khandaq.lnk" "$INSTDIR\uninstall.exe" "" "" ""

  # Write setup/app info into the registry
  ${WriteRegStr} "${REG_ROOT}" "${REG_APP_PATH}" "" "$INSTDIR\${MAIN_APP_EXE}"
  ${WriteRegStr} "${REG_ROOT}" "${REG_APP_PATH}" "Path" "$INSTDIR\bin\"
  ${WriteRegStr} ${REG_ROOT} "${UNINSTALL_PATH}" "DisplayName" "Khandaq"
  ${WriteRegStr} ${REG_ROOT} "${UNINSTALL_PATH}" "DisplayVersion" "1.17.5"
  ${WriteRegStr} ${REG_ROOT} "${UNINSTALL_PATH}" "Publisher" "Khandaq Project"
  ${WriteRegStr} ${REG_ROOT} "${UNINSTALL_PATH}" "UninstallString" "$INSTDIR\uninstall.exe"
  ${WriteRegStr} ${REG_ROOT} "${UNINSTALL_PATH}" "URLInfoAbout" "https://khandaq.org"
  ...
  ${WriteRegStr} "HKCR" "Applications\Khandaq.exe\SupportedTypes" ".tox" ""
...
Section Uninstall
  ...
  RMDir /r /REBOOTOK "$SMPROGRAMS\Khandaq"

  ;Remove registry keys
  DeleteRegKey ${REG_ROOT} "${REG_APP_PATH}"
  DeleteRegKey ${REG_ROOT} "${UNINSTALL_PATH}"
  DeleteRegKey HKCR "Applications\Khandaq.exe"
